    ...
    def run(self, system, config: dict) -> ModuleResult:
        ...
        return ModuleResult(True, output, [
            ...
            (
                ...
                if write_mdp else
                "Dry geometry export requested; GROMACS validation was not required"
            ),
            (
    ...
    def _archive_members(output_dir: Path, *, write_mdp: bool) -> list[Path]:
        ...
        members = {
            output_dir / name for name in (
                "input.gro", "input.pdb", "topol.top", "index.ndx",
                "README.txt", "CITATIONS.json", "manifest.json",
            )
        }
        ...
        return sorted(
            path for path in members if path.is_file() and not path.is_symlink()
        )
