...
class _AmberBase(ForceField):
    ...
    def build_system_topology(self, system: System) -> Topology:
        ...
        try:
            ...
            ff_dir = Path(__file__).resolve().parent.parent.parent / "data" / "forcefields" / f"{self.name}.ff"
            rtp = RTPParser()
            if ff_dir.is_dir():
        ...
        for i in range(system.num_atoms):
            ...
            at = AtomType(
                name=atype_name, mass=params["mass"], charge=charge,
        ...
        for comp in system.components:
            ...
            topology.molecule_blocks.append(MoleculeBlock(
                ...
                nrexcl=nrexcl, type_name=type_name,
                num_molecules=1 if comp.kind != ComponentKind.SOLVENT else comp.metadata.get("n_molecules", 1),
            ))
        return topology
