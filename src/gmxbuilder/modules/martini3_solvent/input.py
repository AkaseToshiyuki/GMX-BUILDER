...
class CGInputModule(BaseModule):
    ...
    def run(self, system, config: dict) -> ModuleResult:
        ...
        if config.get("include_protein", True) is not True:
            raise ModuleConfigError("Martini 3 Solvent Builder requires an uploaded protein")
        config = dict(config)
        config.update({"environment": "solution", "include_protein": True})
        include_protein = strict_bool(config, "include_protein", True)
        environment = str(config.get("environment", "bilayer")).lower()
        output = system.copy()
