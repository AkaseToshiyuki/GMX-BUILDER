...
def test_membrane_padding_is_equal_from_both_lipid_water_interfaces():
    ...
    lower_interface, upper_interface = solvated.metadata["solvation"]["membrane_interface_z_nm"]
...
def test_topology_preserves_noncontiguous_water_runs(tmp_path):
    ...
    TopologyWriter("amber99sb-ildn", {"water_model": "tip3p"}).write_top(structure, topology)
...
def test_prebuilt_water_and_matching_topology_pass_grompp(tmp_path, model_name):
    ...
    TopologyWriter("charmm36m", ff_config={"water_model": model_name}).write_top(
...
def test_exporter_uses_water_model_locked_in_system_metadata(tmp_path):
    selected = (
        ForceFieldSelector()
        .run(_empty_system(), {"name": "charmm36m", "water_model": "spce"})
        .system
    )
    solvated = (
        SolvationBuilder()
...
def test_complete_solvated_membrane_passes_grompp_and_mdrun(tmp_path):
    ...
    TopologyWriter("charmm36m", ff_config={"water_model": "spce"}).write_top(
        solvated.structure, tmp_path / "topol.top"
    )
...
def test_default_solution_protocol_exports_and_passes_grompp(tmp_path):
    ...
    assert (
        ...
        in (output_dir / "mdp" / "equili_1.mdp").read_text()
    )
    assert "DispCorr                = EnerPres" in (output_dir / "mdp" / "equili_1.mdp").read_text()
