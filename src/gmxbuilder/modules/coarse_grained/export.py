...
from gmxbuilder.modules.coarse_grained.protocol import (
    normalize_protocol, write_index, write_mdp_files, write_run_script,
)
...
class CGExportModule(BaseModule):
    ...
    def validate_config(self, config: dict) -> bool:
        self.validate_config_keys(config, {"output_dir", "system_name", "write_mdp", "seed", "_task_dir", "_step_dir"})
    ...
    def run(self, system, config: dict) -> ModuleResult:
        ...
        PDBWriter.write(system.structure, output_dir / "input.pdb", "GMXBUILDER Martini 3", wrap_ids_for_viewer=True)
        ...
        protein_texts = {
            name: text for name, text in dict(system.metadata.get("cg_topology_texts") or {}).items()
        ...
        self._validate_with_gromacs(output_dir, sim, write_mdp=write_mdp)
        manifest = load_manifest()
        readme = [
            "GMXBUILDER Martini 3 simulation package", "",
            f"System: {system_name}", f"Force field: {manifest['force_field']}",
            f"Bundle: {manifest['bundle_id']}", "Water: regular Martini W",
            "",
            *(
                ["Run: ./run_md.sh", "Override GROMACS command: GMX=/path/to/gmx ./run_md.sh"]
                if write_mdp else
                ["Dry bilayer geometry package: no solvated simulation protocol is included."]
            ),
        ...
        (output_dir / "CITATIONS.json").write_text(json.dumps(manifest["citations"], indent=2) + "\n", encoding="utf-8")
        package_manifest = {
            "schema": 1, "resolution": "coarse-grained", "force_field": "Martini 3",
            "bundle_id": manifest["bundle_id"], "beads": system.num_atoms,
            "coordinate_source": "exact cg_system Check checkpoint",
            "simulation_ready": write_mdp,
            "protocol": sim if write_mdp else None,
        }
        (output_dir / "manifest.json").write_text(json.dumps(package_manifest, indent=2) + "\n", encoding="utf-8")
        archive_path = output_dir / f"{sim['system_name']}.zip"
        if archive_path.exists():
            archive_path.unlink()
        with zipfile.ZipFile(archive_path, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=6) as archive:
            for path in sorted(output_dir.rglob("*")):
                if path.is_file() and path != archive_path and not path.is_symlink() and path.name not in {"system.npz", "system.json", "viewer.pdb"}:
        ...
        return ModuleResult(True, output, [
            ...
            (
                ...
                if write_mdp else
                "Wrote a geometry/topology-only dry bilayer package"
            ),
            f"Archive: {archive_path.name}",
        ])

    @staticmethod
    def _validate_with_gromacs(output_dir: Path, sim: dict, *, write_mdp: bool) -> None:
        ...
        if not executable:
            raise ModuleConfigError(
                "GROMACS is required to validate a Martini 3 export"
            )
        with tempfile.TemporaryDirectory(
            ...
            if write_mdp:
                ...
                validation_stages = write_mdp_files(scratch / "mdp", sim)
                if not validation_stages:
                    raise ModuleConfigError("Could not prepare Martini validation MDP")
                mini_mdp = scratch / "mdp" / "mini.mdp"
            run_checked(
                [
                    executable,
                    "grompp",
                    ...
                    "-p", "topol.top",
                    "-n", "index.ndx",
                    "-o", str(scratch / "validation.tpr"),
                    "-po", str(scratch / "validation.mdp"),
                    "-maxwarn", "0",
                ],
                cwd=output_dir,
                timeout=120.0,
